...
class SockStreamConnection:
    ...
    def post_recieve_update(self,
        ...
        if callable(self.post_recieve_func):  # If variable a function
            try:
                ...
                self.warn(
                    f"Error: Post recieve function {self.post_recieve_func.__name__} returned the error {e}")  # Error
    ...
    def enable_outputs(self, boolean):  # Set outputs for warnings and info to active or inactive
        if type(boolean) == bool:  # If input bool
            ...
            self.warn(
                f"Error: Bool {boolean} not valid for setting enable_outputs")  # Error as type of input is not bool
    ...
    def enable_warnings(self, boolean):  # Set outputs for warnings to active or inactive
        if type(boolean) == bool:  # If input bool
            ...
            self.warn(
                f"Error: Bool {boolean} not valid for setting enable_warnings")  # Error as type of input is not bool
...
class Server(SockStreamConnection):
    ...
    def start(self, start=True):
        if not self.active and self.server_addr_avalible():  # Check if server not active and not connected
            self.server = socket.socket(socket.AF_INET,
                                        socket.SOCK_STREAM)  # Creates server on "INET", and sock stream means we are streaming data
            self.server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR,
                                   1)  # Allows server to be reused after shutdown
    ...
    def accept_clients(self, direct):
        if not self.do_shutdown and not self.accepting_clients:  # If server not shutting down and server not accepting clients
            ...
            self.warn("Error: Cannot accept clients because server is shutting down")
        else:
            self.warn(
    ...
    def connection_handler(self, direct):
        ...
        while True:  # Continually adds new connections
            ...
            if direct:
                ...
                thread = threading.Thread(target=self.handle_client_echo, args=(client_conn,
                                                                                client_addr))  # Creates a new thread for this specific connection, with handle_client_echo()
            ...
            self.info("Client Event: Connected")

        self.info("New Connections: Deny")

    def handle_shutdown(self):
        if self.active:
            self.info("Status: Starting Shutdown")
            ...
            self.info("Status: Shutdown")
        else:
            self.warn("Error: Could not shutdown server as it is not online")
    ...
    def direct_send(self, client_conn, message):  # Send a client connected with direct a message
        try:  # Can cause error
            ...
            message = "{'commands':" + str(self.get_commands()) + ", 'data':" + str(
                message) + "}"  # Format message for data

            encoded_message = message.encode(self.FORMAT)  # Encodes message
            encoded_message += b' ' * (self.HEADER - len(
                encoded_message))  # Adds blanks to the end to make the encoded_message the right size
    ...
    def handle_client_direct(self, client_conn,
                             client_addr):  # Given a thread for each connection, gets the clients IP, directly messages client
        ...
        self.info("Client Event: Disconnected")
    ...
    def recieve_message(self, client_conn):  # Recieves message from a client
        client_message_length = client_conn.recv(self.HEADER).decode(
            self.FORMAT)  # Waits for message lenght of next message and decodes from format

        if client_message_length:  # Makes sure that there is a message
            client_message_length = int(client_message_length)
            client_message = client_conn.recv(client_message_length).decode(
                self.FORMAT)  # Waits for message and decodes from format
    ...
    def send_message(self, message, client_conn):  # Send message to a client
        ...
        header_message = str(encoded_message_length).encode(
            self.FORMAT)  # Set header message to the string of the lenght and incode with format
        header_message += b' ' * (self.HEADER - len(
            header_message))  # Adds blanks to the end to make the header_message the right size
        client_conn.send(header_message)  # Header sends the message length of main message
        client_conn.send(encoded_message)  # Sends the message in encoded format

    def handle_client_echo(self, client_conn,
                           client_addr):  # Given a thread for each connection, gets the clients IP, messages client through echos
        self.client_data[client_addr] = ""  # Adds this client to the client_data list with no data
        self.client_conn[client_addr] = client_conn  # Stores connection of client
    ...
    def shutdown(self):  # Disconnect clients and shutdown server
        ...
        thread = threading.Thread(
            target=self.handle_shutdown)  # Creates a new thread for this specific connection, with handle_client_echo()
        thread.daemon = True  # Dies when main thread (only non-daemon thread) exits
        thread.start()  # Starts thread
...
class Client(SockStreamConnection):
    ...
    def __init__(self, server_ip, port, direct=False, outputs_enabled=True, header=64, format='utf-8',
                 pre_send_func=None,
                 post_recieve_func=None):  # Default: header is 64 bytes long, format is utf-8, no functions
    ...
    def connect(self):  # Connect to server
        if not self.active:  # If not already connected
            self.server_conn = socket.socket(socket.AF_INET,
                                             socket.SOCK_STREAM)  # Connects client to INET, streaming data
            ...
            if connected_to_server:
                ...
                if self.DIRECT:
                    thread = threading.Thread(
                        target=self.handle_server_direct)  # Creates a new thread for this specific connection, with handle_server_direct()
                else:
                    thread = threading.Thread(
                        target=self.handle_server_echo)  # Creates a new thread for this specific connection, with handle_server_echo()
    ...
    def handle_server_direct(self):
        ...
        while connected:
            try:  # Recieve can cause error
                ...
                self.warn(
                    "Error: Server disconnected without commanding disconnect, can not recieve message. Disconnecting client."
                )

                self.info("Status: Disconnecting")
                break  # Leave loop
    ...
    def recieve_message(self):  # Recieves message from a client
        client_message_length = self.server_conn.recv(self.HEADER).decode(
            self.FORMAT)  # Waits for message lenght of next message and decodes from format

        if client_message_length:  # Makes sure that there is a message
            client_message_length = int(client_message_length)
            client_message = self.server_conn.recv(client_message_length).decode(
                self.FORMAT)  # Waits for message and decodes from format
            client_message = self.eval_message(client_message)  # Converts data string into dict
            return client_message
    ...
    def handle_server_echo(self):
        ...
        while connected:
            try:  # Recieve can cause error
                ...
                self.warn(
                    "Error: Server disconnected without commanding disconnect. Disconnecting client."
                )
